...
@interface ContactsDataSourceTchap ()
{
    // Search processing
...
@implementation ContactsDataSourceTchap
...
- (void)searchWithPattern:(NSString *)searchText forceReset:(BOOL)forceRefresh hsUserDirectory:(BOOL)hsUserDirectory
{
    ...
    else if (forceRefresh || ![searchText isEqualToString:searchProcessingText])
    {
        ...
        if (!hsUserDirectory)
        {
            _userDirectoryState = ContactsDataSourceTchapUserDirectoryStateOfflineLoading;
            unfilteredMatrixContacts = [self unfilteredMatrixContactsArray];
        }
        else if (![searchText isEqualToString:searchProcessingText])
        {
            _userDirectoryState = ContactsDataSourceTchapUserDirectoryStateLoading;
            ...
            hsUserDirectoryOperation = [self.mxSession.matrixRestClient searchUsers:searchText limit:50 success:^(MXUserSearchResponse *userSearchResponse) {
                ...
                self->_userDirectoryState = userSearchResponse.limited ? ContactsDataSourceTchapUserDirectoryStateLoadedButLimited : ContactsDataSourceTchapUserDirectoryStateLoaded;
            ...
            if (!self->searchProcessingCount)
            {
                if (!self->forceSearchResultRefresh)
                {
                    ...
                    if (!hsUserDirectory)
                    {
                        ...
                        self->_userDirectoryState = ContactsDataSourceTchapUserDirectoryStateOfflineLoaded;
                    }
    ...
    else
    {
        ...
        if (currentSearchText.length)
        {
            if (indexPath.section == filteredMatrixContactsSection &&
                (_userDirectoryState == ContactsDataSourceTchapUserDirectoryStateLoading || _userDirectoryState == ContactsDataSourceTchapUserDirectoryStateOfflineLoading))
            {
...
- (NSAttributedString *)attributedStringForHeaderTitleInSection:(NSInteger)section
{
    ...
    else //if (section == filteredMatrixContactsSection)
    {
        ...
        else
        {
            ...
            {
                case ContactsDataSourceTchapUserDirectoryStateOfflineLoading:
                case ContactsDataSourceTchapUserDirectoryStateOfflineLoaded:
    ...
    if (count)
    {
        NSString *roomCountFormat = (_userDirectoryState == ContactsDataSourceTchapUserDirectoryStateLoadedButLimited) ? @"   > %tu" : @"   %tu";
